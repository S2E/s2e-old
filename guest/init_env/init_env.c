...
int __s2e_range(int start, int end, const char* name) {
  int x;

  if (start >= end)
    __emit_error("__s2e_range: invalid range");
  ...
  if (start+1==end) {
    ...
    if (start==0) {
      if ((unsigned) x >= (unsigned) end) {
        s2e_kill_state(0, "__s2e_range creating a constraint...");
      }
    } else {
      if (x < start || x >= end) {
        s2e_kill_state(0, "__s2e_range creating a constraint...");
...
usage: (s2e_init_env) [options] [program arguments]\n\
  ...
  while (k < argc) {
	...
	__emit_error(msg);
      ...
      n_args = __s2e_range(min_argvs, max_argvs+1, "n_args");
      for (i=0; i < n_args; i++) {
        sym_arg_name[3] = '0' + sym_arg_num++;
        __add_arg(&new_argc, new_argv, 
                  __get_sym_str(max_len, sym_arg_name),
