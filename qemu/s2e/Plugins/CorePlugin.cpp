...
void s2e_on_translate_block_start(
        S2E* s2e, TranslationBlock *tb, uint64_t pc)
{
...
void s2e_on_exception(S2E *s2e, unsigned intNb)
{
    s2e->getCorePlugin()->onException.emit(s2e->getExecutor()->getCurrentState(), intNb);
}
