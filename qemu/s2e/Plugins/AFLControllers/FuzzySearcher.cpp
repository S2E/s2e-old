...
void FuzzySearcher::initialize() {
	...
	//afl related
	m_aflOutputpool = s2e()->getConfig()->getString(cfgkey + ".aflOutput", "", &ok);
	if(!ok){
	...
	m_aflAppArgs = s2e()->getConfig()->getString(cfgkey + ".aflAppArgs", "", &ok);
	if(!ok){
		s2e()->getDebugStream() << "FuzzySearcher: You should specify target binary's full arguments as aflAppArgs\n";
		exit(0);
	}
	m_aflBinaryMode = s2e()->getConfig()->getBool(getConfigKey() + ".aflBinaryMode", false, &ok);
	m_MAXLOOPs = s2e()->getConfig()->getInt(getConfigKey() + ".MaxLoops", 10, &ok);
