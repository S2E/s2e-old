...
namespace klee {
  ...
  public:
    ...
    weight_type getWeight(T item) const;
	
    /* pick a tree element according to its
     * weight. p should be in [0,1).
     */
    T choose(double p) const;
    
  private:
    class Node;
    Node *m_root;
    
    Node **lookup(T item, const Node **parent_out) const;
...
public:
  ...
  bool isBlack() const { return !m_mark; }
  bool leftIsBlack() const { return !left || left->isBlack(); }
  bool rightIsBlack() const { return !right || right->isBlack(); }
  void setSum() {
    sumWeights = weight;
...
template <class T>
T DiscretePDF<T>::choose(double p) const {
  if (p<0.0 || p>=1.0) {
...
template <class T>
bool DiscretePDF<T>::inTree(T item) const {
  Node *n = *lookup(item, 0);
...
template <class T>
typename DiscretePDF<T>::weight_type DiscretePDF<T>::getWeight(T item) const {
  Node *n = *lookup(item, 0);
  assert(n);
  return n->weight;
}
...
DiscretePDF<T>::lookup(T item, const Node **parent_out) const {
  Node *n, *p=0;
  Node * const *np=&m_root;
